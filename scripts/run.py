...
if True:
    ...
    log_dir = 'run_{:02d}{:02d}'.format(now.month, now.day)
...
def route():
    run('/usr/bin/time -v ./{0} -lef {1}.input.lef -def {1}.input.def {2} -threads {3}  -output {4}.solution.def |& tee {4}.log'.format(
